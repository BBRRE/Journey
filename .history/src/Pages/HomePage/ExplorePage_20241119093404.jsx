...
export default function ExplorePage({ criteria }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    const response = await getJourney("journeyData", criteria, null, lastDoc);
    setDisplayedInfo((prev) => [...prev, ...response.data]); // Append new data to the list
    setLastDoc(response.lastDoc); // Update the last visible document
    setLoading(false);
  };

  // Initial fetch
  useEffect(() => {
    setDisplayedInfo([]); // Clear previous results on criteria change
