...
export default function ExplorePage({ criteria }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setHasMore(journeyData.data.length === 4);  // Check if there are more posts to load
    };
  ...
  const loadMore = async () => {
    ...
    setHasMore(journeyData.data.length === 4);  // If less than 4 posts are returned, no more data is available
    setSloading(false)
  };
  return (
    <>
{loading ? (
  ...
  displayedInfo.length === 0 ? (
    ...
    displayedInfo.map(post => (
      ...
      {hasMore && (
            <button
              className={`col-span-2 ${sloading ? 'animate-pulse' : ''}`}
              onClick={loadMore}
