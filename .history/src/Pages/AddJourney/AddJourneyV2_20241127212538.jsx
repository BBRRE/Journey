...
const AddJourneyV2 = () => {
  ...
  const onSubmit = (data) => {
    ...
    // Handle activity images and data
    data.activities.forEach((activity, index) => {
      // Add activity text data
      Object.keys(activity).forEach(key => {
        formData.append(`activities[${index}].${key}`, activity[key]);
      });
  
      // Add activity images
      const activityImages = document.querySelectorAll(`input[name="activities[${index}].images"]`)[0].files;
      Array.from(activityImages).forEach((file, imageIndex) => {
        formData.append(`activities[${index}].images`, file);
      });
    });
