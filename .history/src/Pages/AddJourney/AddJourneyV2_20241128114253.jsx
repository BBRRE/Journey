...
const AddJourneyV2 = () => {
  ...
  useEffect(() => {
    const savedFormData = localStorage.getItem('journeyFormData');
    const savedActivitiesImages = localStorage.getItem('journeyActivitiesImages');
    ...
    if (savedFormData) {
      ...
      // Restore overview image
      if (parsedData.overviewImage) {
        setOverviewImage(parsedData.overviewImage);
      }
    }

    if (savedActivitiesImages) {
      setActivitiesImages(JSON.parse(savedActivitiesImages));
    }
  }, [reset]);
  ...
  useEffect(() => {
    const saveFormData = async () => {
      ...
      localStorage.setItem('journeyFormData', JSON.stringify({
        ...currentValues,
        overviewImage: overviewImage
      }));

      // Save activities images to localStorage
      localStorage.setItem('journeyActivitiesImages', JSON.stringify(activitiesImages));
    };

    // Debounce the save to prevent too frequent localStorage updates
    const timerId = setTimeout(saveFormData, 500);

    return () => clearTimeout(timerId);
  }, [overviewImage, activitiesImages]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      localStorage.removeItem('journeyFormData');
      localStorage.removeItem('journeyActivitiesImages');
