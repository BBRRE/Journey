...
export default function AddJourney() {
      ...
      temp.push(
        ...
        {i > 0 && (
            <div className="flex gap-[10px] flex-col sm:flex-row mb-[10px]">
              <label htmlFor={`transportType${i + 1}`} className="flex flex-col gap-[2px] flex-grow">
                <span className="text-lg">Transport Type</span>
                <select 
              ...
              <label htmlFor={`Descriptiontr${i + 1}`} className="flex flex-col">
              ...
              <label htmlFor={`transportCost${i + 1}`} className="flex flex-col gap-[2px]">
                <span className="text-lg">Transport Cost</span>
                <input
                  type="number"
  ...
  const packageData = (e) => {
    ...
    if (e.target.length < 7) {
      ...
      for (let i = 0; i < activities; i++) {
        const transportOffset = i > 0 ? 2 : 0; // Add 2 input fields for transport if not first activity
        
        const activityData = {
          ActivityNumber: i + 1,
          Name: e.target[inputIndex + transportOffset + 1].value,
          Description: e.target[inputIndex + transportOffset + 2].value,
          Location: e.target[inputIndex + transportOffset + 3].value,
          Price: Number(e.target[inputIndex + transportOffset + 4].value),
          Photos: e.target[inputIndex + transportOffset].files,
        };

        // Add transport information for activities after the first
        if (i > 0) {
          activityData.TransportType = e.target[inputIndex].value;
          activityData.TransportCost = Number(e.target[inputIndex + 1].value);
        }

        formData.push(activityData);
        
        // Update input index based on number of fields
        inputIndex += 5 + transportOffset;
      }
    }
  };
  ...
  const handleSubmit = async (uuid, e) => {
    ...
    let totalPrice = 0;
    const numActivities = activities;
    
    // Sum up prices from activities and transport
    for (let i = 1; i < activities + 1; i++) {
      totalPrice += parseFloat(formData[i].Price) || 0;
      
      // Add transport cost for activities after the first
      if (i > 1) {
        totalPrice += parseFloat(formData[i].TransportCost) || 0;
      }
    }
