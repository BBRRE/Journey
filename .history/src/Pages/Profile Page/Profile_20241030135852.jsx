...
export default function Profile() {
  ...
  const toggleView = () => {
    ...
    // Scroll to the right or left based on the new state
    if (scrollContainerRef.current) {
      scrollContainerRef.current.scrollLeft = isLikedView ? 0 : scrollContainerRef.current.scrollWidth / 2;
    }
  };
...
<div className='flex flex-col w-full h-auto mt-12 '>
  <div className='w-full border-b-2 border-blue-gray-900 flex justify-between px-12'><h1 onClick={() => {setView(true)}}>My posts</h1><h1 nClick={() => {setView(false)}}>Liked posts</h1></div>
            
            <div className={`w-full overflow-x-hidden  `}>
              <div className='w-[200%] flex overflow-x-scroll'   ref={scrollContainerRef}>
