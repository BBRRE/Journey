...
export async function addLikeToUser(journeyId, uid, username, userEmail) {
  try {
    const userLikesRef = doc(db, "users", userEmail, "likes", uid);
...
export async function removeLikeFromUser(journeyId,uid, userEmail) {
  try {
    const likeDoc = doc(db, "users", userEmail, "likes", uid);
