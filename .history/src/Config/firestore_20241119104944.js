...
//fetch data
export async function getJourney(collectionName, criteria = null, activity = false, lastVisible) {
  const allData = [];
  ...
  if (activity) {
    ...
    let baseQuery = query(
      ...
      orderBy("likes"),
      limit(4)  // Limiting to 4 posts per fetch
    );
    ...
    return { data: allData, lastVisibleDoc };
  } else {
    ...
    if (lastVisible) {
      baseQuery = query(defaultQuery, startAfter(lastVisible));  // Pagination: start after last visible document
    }
