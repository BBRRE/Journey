...
export async function getJourney(collectionName, criteria, lastDoc) {
  const { searchTerm, filters } = criteria || {};

  const queryConstraints = [];
  queryConstraints.push(orderBy("name")); // Adjust orderBy field as needed
  queryConstraints.push(limit(12)); // Limit to 12 items per query
  ...
  if (filters) {
    ...
    if (continent) {
      queryConstraints.push(where("Continent", "==", continent));
    }

    if (minPrice && maxPrice) {
      queryConstraints.push(where("totalPrice", ">=", minPrice), where("totalPrice", "<=", maxPrice));
    }
  }

  // Fetch the documents with the query
  const journeyQuery = query(collection(db, collectionName), ...queryConstraints);
  ...
  for (const docSnap of querySnapshot.docs) {
    ...
    // Fetch the activity subcollection for each post
    const activityCollectionRef = collection(db, collectionName, docSnap.id, "activity");
    const activitySnapshot = await getDocs(activityCollectionRef);
    ...
    lastVisible = docSnap; // Keep track of the last visible document for pagination
  }

  return { data: allData, lastVisible };
}
