...
//fetch data
export async function getJourney(collectionName, criteria, activity) {
  const allData = [];
  ...
  if (criteria) {
    ...
    // Build the queries for OR logic
    const nameQuery = query(
      collection(db, collectionName),
      where("name", "array-contains", lowerSearchTerm),
    ...
    const locationQuery = query(
      collection(db, collectionName),
      where("location", "array-contains", lowerSearchTerm),
      where("price", ">=", minPrice),
      where("price", "<=", maxPrice),
      where("Continent", "==", continent),
      where("numActivities", ">=", minActivities)
    ...
    const userNameQuery = query(
      collection(db, collectionName),
      where("userName", "array-contains", lowerSearchTerm),
      where("price", ">=", minPrice),
      where("price", "<=", maxPrice),
      where("Continent", "==", continent),
      where("numActivities", ">=", minActivities)
    ...
    const mergedDocs = new Map(); // Use a Map to remove duplicates by ID

    for (const querySnapshot of querySnapshots) {
      querySnapshot.forEach((docSnap) => {
        if (!mergedDocs.has(docSnap.id)) {
          mergedDocs.set(docSnap.id, docSnap);
        }
      });
    }

    // Process the unique results
    for (const docSnap of mergedDocs.values()) {
    ...
    // Sort by likes and limit to 2 results
    allData.sort((a, b) => b.likes - a.likes);
    console.log(allData.slice(0, 2)); // Debugging
    return allData.slice(0, 2);
  } else {
    // Default case with no criteria
