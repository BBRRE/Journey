...
//fetch data
export async function getJourney(collectionName, criteria, activity, lastDoc = null) {
  const allData = [];
  const limitValue = 10; // Number of documents per page
  ...
  if (activity) {
    ...
    console.log(allData);
    return { data: allData, lastDoc: querySnapshot.docs[querySnapshot.docs.length - 1] };
  } else {
    // Default case: No criteria
    const defaultQuery = query(
      collection(db, collectionName),
      orderBy("likes"),
      lastDoc ? startAfter(lastDoc) : undefined, // For pagination
      limit(limitValue)
    );
