...
//fetch data
export async function getJourney(collectionName, criteria, activity, lastVisibleDoc = null) {
  const allData = [];
  
  if (activity) {
    // Fetch activity data (no changes needed for pagination here)
    const defaultQuery = query(collection(db, collectionName));
    ...
    // Base query for filters with pagination support
    let baseQuery = query(
      ...
      orderBy("likes"),
      limit(10) // Fetch more documents for pagination
    );
    ...
    if (lastVisibleDoc) {
      baseQuery = query(baseQuery, startAfter(lastVisibleDoc)); // Start after the last document of the previous page
    }
    ...
    // Return the filtered data and the last document for pagination
    const lastVisible = querySnapshot.docs[querySnapshot.docs.length - 1]; // Get the last document to use for pagination
    console.log(allData);
    return { filteredData: allData.slice(0, 2), lastVisible }; // Limit the results to 2
  } else {
    // Default case: No criteria, fetch top 2 journeys by likes with pagination support
    const defaultQuery = query(
      collection(db, collectionName),
      orderBy("likes"),
      limit(10) // Limit to 10 documents for pagination
    );

    if (lastVisibleDoc) {
      baseQuery = query(defaultQuery, startAfter(lastVisibleDoc)); // Start after the last document of the previous page
    }
    ...
    // Return the top journeys and the last document for pagination
    const lastVisible = querySnapshot.docs[querySnapshot.docs.length - 1]; // Get the last document to use for pagination
    console.log(allData);
    return { filteredData: allData.slice(0, 2), lastVisible }; // Limit to top 2
  }
}
