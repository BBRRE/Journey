import { Firestore, addDoc, collection, doc, getDoc, getDocs, query, serverTimestamp,  orderBy, limit, startAfter   } from "firebase/firestore";
...
//fetch data
export async function getJourney(collectionName, criteria = null, lastDoc = null) {
  // Destructure criteria if provided
  const { searchTerm, filters } = criteria || {};
  const { minPrice, maxPrice, continent, minActivities } = filters || {};

  // Start with the base query
  let journeyQuery = collection(db, collectionName);
  ...
  journeyQuery = query(journeyQuery, ...queryConstraints);

  // Execute the query
  const querySnapshot = await getDocs(journeyQuery);
  const allData = [];

  // Prepare search term for case-insensitive search if provided
  ...
  for (const docSnap of querySnapshot.docs) {
    ...
    const matchesSearch = !lowercaseSearchTerm || searchFields.some(field =>
      data[field]?.toLowerCase().includes(lowercaseSearchTerm)
    );

    // Skip if doesn't match search
    if (!matchesSearch) continue;

    // Check continent filter if provided
    const matchesContinent = !continent || data.Continent === continent;
    if (!matchesContinent) continue;

    // Fetch and calculate activities data
    ...
    // Apply price and activity filters
    const matchesPrice = (!minPrice || totalPrice >= minPrice) && 
                        (!maxPrice || totalPrice <= maxPrice);
    const matchesActivities = !minActivities || numActivities >= minActivities;

    // Only add document if it matches all filters
    if (matchesPrice && matchesActivities) {
      allData.push({
        ...data,
        id: docSnap.id,
        totalPrice,
  ...
  const hasMore = querySnapshot.docs.length === 2;
