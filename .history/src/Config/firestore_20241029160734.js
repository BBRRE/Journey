...
export async function getJourney(collectionName, searchTerm) {
  ...
  // Check if searchTerm is defined for search functionality
  ...
  for (const doc of querySnapshot.docs) {
    ...
    const totalPrice = activities.reduce((acc, activity) => acc + (activity.price || 0), 0);
    const numActivities = activities.length;

    // Check if this document matches the search term
    const matchesSearch = lowercaseSearchTerm
      ? searchFields.some(field =>
          data[field] && data[field].toLowerCase().includes(lowercaseSearchTerm)
        )
      : true; // No search term, include all documents
