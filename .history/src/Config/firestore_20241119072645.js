...
//fetch data
export const getFilteredJourneys = functions.https.onCall(async (data, context) => {
  const { collectionName, searchTerm = "", filters = {} } = data;
  ...
  try {
      ...
      querySnapshot.docs.map(async (doc) => {
        ...
        const matchesSearch = searchTerm
          ? searchFields.some((field) =>
              docData[field]?.toLowerCase().includes(searchTerm.toLowerCase())
            )
          : true;

        if (!matchesSearch) return null;

        // Fetch activity subcollection and calculate totals
        ...
        const activities = activitySnapshot.docs.map((activityDoc) => activityDoc.data());
        const totalPrice = activities.reduce((sum, activity) => sum + (activity.price || 0), 0);
        const numActivities = activities.length;

        // Apply filters
        const { minPrice = 0, maxPrice = Infinity, continent, minActivities = 0 } = filters;
        const matchesPrice = totalPrice >= minPrice && totalPrice <= maxPrice;
        const matchesContinent = continent ? docData.Continent === continent : true;
        const matchesActivities = numActivities >= minActivities;

        if (matchesPrice && matchesContinent && matchesActivities) {
          return {
            ...docData,
            id: doc.id,
            totalPrice,
    ...
    console.error("Error fetching filtered journeys:", error);
    throw new functions.https.HttpsError("internal", "Unable to fetch journeys.");
  }
});
