...
//fetch data
  ...
  const querySnapshot = await getDocs(journeyQuery);
    ...
    const journeyQuery = query(
      collection(db, collectionName,or(where('name', 'array-contains', lowerSearchTerm ),where('loaction', 'array-contains', lowerSearchTerm ),where('userName', 'array-contains', lowerSearchTerm ),where('description', 'array-contains', lowerSearchTerm )),where('price', '>=', minPrice),where('price', '<=', maxPrice )
      ,where(continent)),
      orderBy('likes'),
      limit(2)
  );
