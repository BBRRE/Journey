...
//fetch data
export async function getJourney(collectionName, criteria, activity, lastVisible = null) {
  const allData = [];
  let baseQuery;
  ...
  if (activity) {
    ...
    if (lastVisible) {
      baseQuery = query(baseQuery, startAfter(lastVisible)); // Start after the last document
    }
    ...
    return { data: allData, lastVisible: querySnapshot.docs[querySnapshot.docs.length - 1] };
  } else {
    // Default case: No criteria, fetch top 4 journeys by likes
    baseQuery = query(
      collection(db, collectionName),
      orderBy("likes"),
      limit(6)
