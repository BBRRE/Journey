...
export async function getJourney(collectionName, criteria = null, activity = false, lastVisible = null) {
  ...
  if (activity) {
    ...
    querySnapshot.forEach(async (docSnap) => {
      ...
      allData.push({ ...data, id: docSnap.id, imageURL: await retrieveImages(data["imageBucket"]) });
    });
    return { data: allData, lastVisibleDoc: null }; // Return data and null for pagination if no pagination logic
  } else if (criteria) {
