...
export async function getLikedPosts(userEmail) {
  try {
    ...
    // Get all docIds from the likes documents (note the lowercase 'd')
    const likedPostIds = likesSnapshot.docs.map(doc => doc.data().docId);
    ...
    const fetchPostPromises = likedPostIds.map(async (postId) => {
      if (!postId) {
        console.log('Warning: Found null or undefined postId');
        return null;
      }
