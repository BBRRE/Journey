...
<Link to={`/${userData?.username}`} className={`${user ? 'block' : 'hidden' }`} > 
  ...
  onError={(e) => { e.target.src = "public/assets/Default_pfp.svg.png"; }} />
      ...
      <div className="w-[95%] bg-white shadow-md rounded-lg border border-gray-200 py-4 px-5 mx-auto flex flex-col my-4 text-gray-800 font-sans font-medium text-lg gap-3">
        {user ? (
          <>
            <Link to="/" className={`flex items-center gap-2 py-2 mx-2 hover:bg-blue-gray-100 rounded transition ${currentPage === 'home' ? 'bg-blue-gray-100' :  '' }`}>
              <img src="public/assets/Compass.svg" className="w-6" alt="Explore Icon" />
              <span>Explore Page</span>
            </Link>
            <Link to="/AddJourney" className="flex items-center gap-2 py-2 hover:bg-blue-gray-100 rounded transition">
