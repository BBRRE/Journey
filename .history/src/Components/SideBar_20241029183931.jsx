...
export const SideBar = ({ currentPage, onSearchAndFilterApply }) => {
  ...
  return (
    <div className="hidden 2xl:block 2xl:w-[350px] h-full rounded-l-3xl fixed left-0 top-0 ">
      <div className="flex flex-col pb-3 bg-white shadow-md rounded-r-3xl p-6">
        <Link to={`/${userData?.username}`} className={`${user ? 'block' : 'hidden'}`} >
          <div className="flex w-[90%] max-w-[1160px] mx-auto mt-[20px]">
            <img className="w-[80px] h-[80px] rounded-full shadow-md" src={userData?.photoURL || "path/to/default-image.jpg"}
              alt="Profile"
              onError={(e) => { e.target.src = "public/assets/Default_pfp.svg.png"; }} />
            <div className="flex flex-col pl-4 w-[250px]">
              ...
              <span className="text-sm text-gray-600 mt-2">{userData?.bio}loremipsuafdsfsdfsdfsdfsd sd fsd sdfsdfsdf sdfsdf sdfsd sdf </span>
            </div>
          </div>
        </Link>
      </div>
      ...
      <div className="w-[95%] bg-white shadow-md rounded-lg border border-gray-200 py-4 px-5 mx-auto flex flex-col my-4 text-gray-800 font-sans font-medium text-lg gap-3">
        {user ? (
          <>
            <Link to="/" className={`flex items-center gap-2 py-2 hover:bg-blue-gray-100 rounded transition ${currentPage === 'home' ? 'bg-blue-gray-100  px-2' : ''}`}>
            ...
            <div className='flex flex-col text-md ml-[-10px] gap-1 text-blue-gray-800'>
              ...
              <Settings>
                <div className="flex items-center gap-2 py-2 hover:bg-blue-gray-100 rounded transition cursor-pointer ">
                  <img src="public/assets/settings-svgrepo-com.svg" className="w-5" alt="Settings Icon" />
                  <span>Settings</span>
                </div>
              </Settings>
              <Link onClick={handleSignOut} className="flex items-center gap-2 py-2 hover:bg-blue-gray-100 rounded transition">
                <img src="public/assets/logout-2-svgrepo-com.svg" className="w-5" alt="Logout Icon" />
                <span>Log Out</span>
              </Link>
