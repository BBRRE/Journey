...
export default function Overview(props) {
  ...
  const handleLike = async () => {
    try {
      ...
      if (isLiked) {
        // Unlike the post
        await removeLikeFromUser(props.obj.id, user.email);
        await updateJourneyLikes(props.obj.id, likes - 1);
        setLikes(likes - 1);
        setIsLiked(false);
      } else {
        // Like the post
        const likeStatus = await checkIfUserLikedPost(props.obj.id, user.email);
        if (!likeStatus) {
          await addLikeToUser(props.obj.id, props.obj.username, user.email);
          await updateJourneyLikes(props.obj.id, likes + 1);
          setLikes(likes + 1);
          setIsLiked(true);
        }
