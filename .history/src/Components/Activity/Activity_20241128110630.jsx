  ...
  return (
    <div className='w-full flex relative'>
      ...
      <div className='flex mx-auto flex-col gap-[20px]'>
        ...
        <div className={`bg-[#F5F4F4] w-full p-[20px] mb-11 rounded-md max-w-[800px] flex flex-col mx-auto shadow-md ${processedActivities[1].transportType ? '' : 'gap-8'}`}>
          <div className='ml-auto pr-4 text-2xl mb-4' >Day 1</div>
          {loading ? <ActivitySceleton /> : (processedActivities.map((activity, index) => (
            <div key={index} className='flex flex-col w-full gap-[10px]'>
              {index > 0 && activity.transportType && (
                (
                  <div className={`flex justify-between items-center px-2   ${activity.dayNumber === processedActivities[index - 1].dayNumber || activity.dayNumber === 1 ? 'border-l-2 border-dashed border-gray-600 py-10' : 'pt-10'} `} >
                    ...
                    </div> <div className={`pr-4 text-2xl ml-auto mt-8 ${activity.dayNumber === processedActivities[index - 1].dayNumber || activity.dayNumber === 1 ? 'hidden' : ''} `}> Day {activity.dayNumber}</div>
                  </div>)
              )}
              <div className={`w-full `}> {/* Container for images */}
                {renderNestedImages(activity.processedImages)}
              </div>
              <div className={`gap-[2px] flex flex-col  ${processedActivities[1].transportType ? '' : 'gap-8'} `}>
