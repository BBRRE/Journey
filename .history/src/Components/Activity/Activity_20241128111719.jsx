    ...
    <div className='w-full flex relative'>
      ...
      <div className='flex mx-auto flex-col gap-[20px]'>
        ...
        <div className='bg-[#F5F4F4] w-full p-[20px] mb-11 rounded-md max-w-[800px] flex flex-col mx-auto shadow-md'>
          <div className='ml-auto pr-4 text-2xl mb-4 w-[100px] ' >Day 1</div>
          {loading ? <ActivitySceleton /> : (processedActivities.map((activity, index) => (
            <div key={index} className='flex flex-col w-full gap-[10px]'>
              {index > 0 && activity.transportType && (
                (
                  <div className={`flex flex-col-reverse justify-between items-center px-2 -mb-2 mt-4 ${activity.dayNumber === processedActivities[index - 1].dayNumber || activity.dayNumber === 1 ? 'border-l-2 border-dashed border-gray-600 py-10' : 'pt-10'} `} >
                    ...
                    <div className={`pr-4 w-[100px] text-2xl ml-auto mb-12 ${activity.dayNumber === processedActivities[index - 1].dayNumber || activity.dayNumber === 1 ? 'hidden' : ''} `}> Day {activity.dayNumber}</div>
                  </div>)
              )}
