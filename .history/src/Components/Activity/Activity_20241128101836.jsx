      ...
      <div className='bg-[#F5F4F4] w-full p-[20px] mb-11 rounded-md max-w-[800px] flex flex-col mx-auto shadow-md'>
        {loading ?  <ActivitySceleton/> : (processedActivities.map((activity, index) => (
          <div key={index} className='flex flex-col w-full gap-[10px] mb-10'>
            {index > 0 && (
              <p>{activity.dayNumber}|{activity.transportCost}|{activity.transportDescription}|{activity.transportType}</p>
              <div className='flex justify-end' > <div className='flex flex-col'><div className='flex'><div>{activity.transportType}</div>${activity.transportCost}<div></div>
              </div>{activity.transportDescription}</div>  </div>
            )}
