          ...
          {loading ? <ActivitySceleton /> : (processedActivities.map((activity, index) => (
            <div key={index} className='flex flex-col w-full gap-[10px]'>
              {index > 0 && activity.transportType && (
                (
                  <div className={`flex justify-between items-center px-2 -mb-4 mt-4 ${activity.dayNumber === processedActivities[index - 1].dayNumber || activity.dayNumber === 1 ? 'border-l-2 border-dashed border-gray-600 py-10' : 'pt-10'} `} >
                    <div className='flex flex-col text-md'>
                      ...
                      {activity.transportDescription} das dasd asdsadasd asdasd asdas adasd fdsfsdf sdfdsf sdf sd fdsfdsf sdfsdf dfdf dfd 
                    </div> <div className={`pr-4 text-2xl ml-auto mb-12 ${activity.dayNumber === processedActivities[index - 1].dayNumber || activity.dayNumber === 1 ? 'hidden' : ''} `}> Day {activity.dayNumber}</div>
                  </div>)
              )}
