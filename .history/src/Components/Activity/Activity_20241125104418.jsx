...
function ActivityPage() {
  ...
  useEffect(() => {
    const fetchActivityData = async () => {
      try {
        ...
        setProcessedActivities(activitySnapshot || [])
        setLoading(false)
  ...
  return (
    ...
    <div className='flex mx-auto flex-col  gap-[20px]'>
      <div onClick={ () => {navigate(`/${userName}`)}} className='flex mx-auto mt-[50px]'>
      ...
      <div className='bg-[#F5F4F4] w-full p-[20px] mb-11 rounded-md max-w-[800px] flex flex-col mx-auto shadow-md'>
        {loading ?  <ActivitySceleton/> :  processedActivities && processedActivities.length > 0 ? (processedActivities.map((activity, index) => (
        ...
        ))) : (<p className='mx-auto' >No Activity Data :(</p>)}
      </div>
